...
Initialises vendored submodules, builds libwaku and compiles the Waku module plugin.
  --debug        Build with Debug configuration
  --release      Build with Release configuration (default)
  --clean        Remove the existing build directory and libwaku artifacts before configuring
...
if [[ ${CLEAN_FIRST} -eq 1 ]]; then
    ...
    echo "Cleaning libwaku artifacts"
    rm -rf "${ROOT_DIR}/lib"
fi
...
echo "Building libwaku from vendored sources..."
if [[ ${CLEAN_FIRST} -eq 1 ]]; then
    "${ROOT_DIR}/build_libwaku.sh" --clean
else
    "${ROOT_DIR}/build_libwaku.sh"
fi
...
echo "Building waku_module_plugin..."
cmake --build "${BUILD_DIR}" --target waku_module_plugin --config "${BUILD_TYPE}"
...
BIN_PATH="${BUILD_DIR}/modules/waku_module_plugin${LIB_SUFFIX}"
if [[ -f "${BIN_PATH}" ]]; then
